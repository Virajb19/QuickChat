...
export const userRouter = createTRPCRouter({
  ...
  createChat: protectedProcedure.input(createChatSchema).mutation(async ({ctx,input}) => {
      ...
      // const chatExists = await ctx.db.chat.findUnique({ where: { passcode: hashedPasscode}, select: { id: true}})
      // if(chatExists) throw new TRPCError({ code: 'FORBIDDEN', message: 'chat with that passcode already exists'})

      const chat = await ctx.db.chat.create({ data: { title, passcode: hashedPasscode ,ownerId: userId}, select: { id: true}})
